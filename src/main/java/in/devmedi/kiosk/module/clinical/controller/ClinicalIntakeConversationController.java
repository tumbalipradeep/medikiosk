 ...
 * completes after the eighth Ahara-Vihara parameter. Answers are not persisted
 * and no AI providers are used.</p>
 */
@RestController
...
public class ClinicalIntakeConversationController {
    ...
    public ClinicalIntakeResponse start() {
    ...
    public ClinicalIntakeResponse answer(@RequestBody ClinicalConversationAnswerRequest request) {
        ...
        if (clinicalQuestionIds.contains(questionId)) {
            return handleClinicalAnswer(questionId, request.answer());
        }
        if (dashavidhaQuestionIds.contains(questionId)) {
            return handleDashavidhaAnswer(questionId, request.answer());
        }
        if (aharaViharaQuestionIds.contains(questionId)) {
            return handleAharaViharaAnswer(questionId, request.answer());
        }
